...
def edd(request):
	...
	if  request.method == "POST":
		...
		if b.is_valid():
			...
			return redirect('/')
	b = EForm()
	return render(request,'bt/emplist.html',{'t':b,'h':g})

def empp(request,c):
	k = Employee.objects.get(id=c)
	if request.method == "POST":
		t = EForm(request.POST,instance=k)
		if t.is_valid():
			t.save()
			return redirect('/')
	t = EForm(instance=k)
	return render(request,'bt/empd.html',{'n':t})